...
def fit_naive_bayes_model(matrix, labels):
    ...
    """
    # Calculate prior probability of y
    m, V = matrix.shape

    y_0 = matrix[labels == 0, :].sum(axis=0)
    y_1 = matrix[labels == 1, :].sum(axis=0)

    # Calculate parameters
    phi_k_y0 = (y_0 + 1) / (y_0.sum() + V)
    phi_k_y1 = (y_1 + 1) / (y_1.sum() + V)
...
def predict_from_naive_bayes_model(model, matrix):
    ...
    p0_y = matrix.dot(np.log(phi_k_y0)) + np.log(1 - phi_y)
    p1_y = matrix.dot(np.log(phi_k_y1)) + np.log(phi_y)

    return (p1_y > p0_y).astype(int)
...
def get_top_five_naive_bayes_words(model, dictionary):
    ...
    """
    phi_k_y1, phi_k_y0, phi_y = model
    # word_index = np.log(phi_k_y1 / phi_k_y0).argsort()[::-1][:5]
    word_index = (-np.log(phi_k_y1 / phi_k_y0)).argsort()[:5]   # descending order
...
if __name__ == "__main__":
    ...
    # Fit NB MEM and make predictions on test set
    ...
    # Compute the best radius for SVM
    optimal_radius = compute_best_svm_radius(train_matrix, train_labels, val_matrix, val_labels, [0.01, 0.1, 1, 10])
    util.write_json('./output/p06_optimal_radius', optimal_radius)
    print('The optimal SVM radius was {}'.format(optimal_radius))

    # Train the model with best radius
